...
class TestRunAll:
    ...
    def test_json_output_schema_validation(self):
        json_text = '{"name": "test", "value": 42, "tags": ["a","b"]}'
        spec = {
            "output_schema": {"name": "string", "value": "number", "tags": "list"},
            "required_fields": ["name", "value"],
        }
        f = dc.run_all(json_text, spec)
        assert len(f) == 0

    def test_json_output_schema_failure(self):
        json_text = '{"name": 123, "value": "not_a_number"}'
        spec = {
            "output_schema": {"name": "string", "value": "number"},
        }
        f = dc.run_all(json_text, spec)
        assert len(f) == 2  # both fields wrong type

    def test_json_output_missing_field(self):
        json_text = '{"name": "test"}'
        spec = {
            "output_schema": {"name": "string", "value": "number"},
        }
        f = dc.run_all(json_text, spec)
        assert len(f) == 1  # value missing
