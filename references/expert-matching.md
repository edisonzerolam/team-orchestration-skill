...
### 专家池渐进式披露（v3.5 增强 · P2-5）

> 在 T0/T1/T2 惰性机制之上，补**阶段内渐进**：立案时先注入最小集（T1 团队头寸 + knowledge 最小集），阶段推进**按需追加**（如质证中发现缺某领域视角，再补注入相关 agent 人设），不做全量披露——省上下文且避免立场污染。与 SKILL.md §6 同口径。

- **追加时机**：B 举证收齐后按争点覆盖盲区追加；C 质证出现新分歧域时追加对应专家视角；追加人设需在质证记录标注（可审计）。
- **约束**：追加不改变已定角色边界（§6 角色分配）；追加视角产物并入质证，不重开已完成的阶段（遵循 C2 固定轮次语义）。
