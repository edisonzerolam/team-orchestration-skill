...
1. **建立团队**：任务开始时由主理人亲自创建本次任务的团队（建议命名 `research-<课题简称>`，如 `research-ai-agent-enterprise`、`research-new-energy-trend`），明确本次协作的边界与上下文。**团队创建（通过 task 工具调度）必须且只能由主理人执行，严禁委派任何成员创建团队**
...
- 轮次由主理人在每次调度明鉴秋时显式传入 `current_round = N / 3`
- 第 3 轮明鉴秋强制返回 PASS
