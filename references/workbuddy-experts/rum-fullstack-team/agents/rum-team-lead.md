...
3. 调度时在 task 工具的 `name` 参数中传入成员的中文角色名（如 "RUM 接入官"、"RUM 性能分析师"），便于用户界面识别
...
1. **建立团队**：任务开始时由主理人亲自创建本次任务的团队（建议命名 `rum-<任务简称>`），明确本次协作的边界与上下文。**团队创建（通过 task 工具调度）必须且只能由主理人执行，严禁委派任何成员创建团队**
...
- `name: "rum-integration-specialist", subagent_type: "general-purpose"`
- `name: "rum-performance-analyst", subagent_type: "general-purpose"`
