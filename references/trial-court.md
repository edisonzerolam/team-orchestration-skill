...
> **现行补充协议（Trial Court）**：本文档为团队编排的**完整审判庭协议细节**。SKILL.md v3.4 五阶段（立案→并行举证→质证→一审→二审终审）是完整协议的精简视图；本文档为完整协议的展开执行规范（含案卷归档、自学习 S1/S2、终审七段式、二审终审制）。

> team-orchestration v3.4.0 默认运行机制。每一议题满足触发条件即走审判庭五阶段，
...
| 原则 | 说明 |
|------|------|
| **审判庭优先** | 满足触发条件即默认走五阶段（非可选），原 10 步工作流映射为其子阶段 |
...
│ Phase A 立案&争点拆解 ─► Phase B 举证(并行) ─► Phase C 质证(并行) ─► Phase D 一审(裁决+回灌修订) ─► Phase E 二审终审(独任) │
└──────────────────────────────┬──────────────────────────────────────┘
                               ▼
...
Phase C 质证（子代理并行）
  ...
  产出 P{i}(B)；争议未收敛自动追加一轮（最多 2 轮）→ 落盘 02-质证/
Phase D 一审裁决 + 回灌修订（main 裁决 → 子代理并行修订）
  main 逐条采信/排除 → 《一审判决书》（中间产物）→ 回灌判决书+各方产物 → 子代理逐条回应判决并修订立场
  产出 P{i}(C)（固定 1 轮，不追加不提前）→ 落盘 03-一审/ + 04-回灌修订/
Phase E 二审终审裁决（main 独任，不回灌）
  汇总 P{i}(C) → 终局裁断 → 《终审意见书》（二审终局）→ 记录资产使用 → 触发自学习归档
  落盘 05-二审终审/ → 归档 deliverables/trial/archive/{yyyy-mm}/{docket_id}/
```
...
- 收齐后判断收敛：分歧点 ≤1 → 进 Phase D（一审）；>1 → 追加一轮（最多 2 轮）
- 自动触发 Layer 1.5 交叉验证：多来源主张检查来源独立性，单来源标注"中等置信度"，冲突点标注"分歧未收敛"

### 4.4 Phase D 一审要点（裁决独任 + 回灌修订并行）
- **一审裁决（main 独任）**：汇总最新一轮 `P{i}(B)`，对每条子争点逐条裁断：采信 / 部分采信 / 排除 → 《一审判决书》（中间产物）
- **回灌修订（子代理并行，固定 1 轮）**：把《一审判决书》+ 各方最新产物回灌给每个子代理 → 逐条回应判决（服判/异议+理由）+ 立场再修订 → P{i}(C)
- 一审判决书为**中间产物**：落盘 `03-一审/first-instance-verdict.md`，流程中向用户展示摘要，不单独交付、不停顿
- 二审必审：不因一审结论被普遍接受而跳过二审

### 4.5 Phase E 二审终审要点（main 独任，不回灌）
- 汇总 `P{i}(C)`，对每条子争点逐条终局裁断：采信 / 部分采信 / 排除（禁止引入新论点）
- 终审意见书结构：① 案卷信息 ② 争点回顾 ③ 质证与一审修订记录 ④ 逐条裁决 ⑤ 终局结论 ⑥ 存疑/遗留事项 ⑦ 资产使用评价
- 二审为终局，**不再回灌修订**；终审结论作为主工作流"交付"环节输入，触发自学习归档（Layer S1+S2）
...
### 5.3 回灌修订 Prompt（Phase D · 一审后二审前）
```markdown
## 阶段 D: 一审回灌修订 — {角色名}

一审已结束。审判长把《一审判决书》和各方最新产物全部交给你，请为二审终审做最后一轮修订。
...
### 三、给二审审判长的终局建议
```
（本轮回灌修订为**固定 1 轮**：不因收敛提前结束，也不追加第二轮，修订后直接进入二审终审）
...
### 6.3 质证轮次控制
- 质证默认 1 轮；`max_rounds > 1` 且分歧未收敛（>2 个关键点）→ 自动追加；**最大 2 轮**；终审保留所有轮次记录
- 一审回灌修订**固定 1 轮**（不追加、不因收敛提前），修订后直接进入二审终审（不再回灌）
...
def cross_examination_phase(docket, evidence_map, sub_agents, max_rounds=2) -> dict  # Phase C 质证（默认 1 轮，最多 2 轮）
def first_instance_phase(docket, final_evidence) -> Verdict  # Phase D 一审裁决（产出《一审判决书》，随后回灌修订 1 轮）
def final_instance_phase(docket, revised_evidence) -> Verdict  # Phase E 二审终审（终局，不回灌）
...
| ⑧ 自验证+审核 | Phase D 一审裁决 + 回灌修订 + Phase E 二审终审（Layer 1+2） | main + 子代理 |
